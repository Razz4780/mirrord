...
pub struct S3CopySpec {
    ...
    /// Regular expressions matched against the source objects' keys. An object is copied when
    /// it matches any of them. Unset copies every object.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    fn from(config: S3BranchCopyConfig) -> Self {
        match config {
            ...
            S3BranchCopyConfig::All { objects } => S3CopySpec {
                ...
                objects: objects.filter(|objects| !objects.is_empty()),
            },
        }
    }
}
...
mod tests {
    ...
    fn s3_copy_spec_carries_object_patterns() {
        ...
        assert_eq!(empty.objects, None);

        let all = S3CopySpec::from(S3BranchCopyConfig::All {
            objects: Some(vec!["^fixtures/.*".to_owned()]),
        });
        assert!(matches!(all.mode, S3BranchCopyMode::All));
        assert_eq!(all.objects, Some(vec!["^fixtures/.*".to_owned()]));

        let all_of_them = S3CopySpec::from(S3BranchCopyConfig::All {
            objects: Some(vec![]),
        });
        assert!(matches!(all_of_them.mode, S3BranchCopyMode::All));
        assert_eq!(all_of_them.objects, None);
    }
