...
mod tests {
    ...
    #[rstest]
    #[case::no_bucket(json!({}), "is required")]
    #[case::unknown_param(json!({ "bucket": "BUCKET", "table": "TABLE" }), "`table` is not a valid")]
    #[case::fixed_slot(json!({ "bucket": "BUCKET", "host": "HOST" }), "`feature.db_branches[].source.params.host` is not a valid")]
    fn s3_verify_rejects_params_other_than_bucket(
        #[case] params: Value,
        ...
        let error = DatabaseBranchesConfig(vec![branch])
            .verify(&mut config::ConfigContext::default())
